...
#Prce per sq foot positive feedback loop. one is calculated from the other. Price and price per sq foot. this value is exactly dervied from price. calculated from the price outcome variable. Model to predict price. using a variable to perfectly match the price outcome. 
...
# c. Check levels:
#Use levels() reassign so that reference is regular sale

# d. Reassign reference level of "Status" to "Regular":
#fct_relevel() function in forcats package in tidyverse
...
#Probably the first one. but this is dangerous. NOT justification for choosing the first model. doesn't make sense. 
```
